...
class System_control extends MS_Controller 
{
    ...
    public function update_content($message = '')
    {
        ...
        $this->form_validation->set_rules('slider_text2_title[0]', 'Slider Text2', 'required');
        $this->form_validation->set_rules('slider_text3_title[0]', 'Slider Text3', 'required');
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $slider_text2_titles = $this->input->post('slider_text2_title');
            $slider_texts = $this->input->post('slider_text2');
            if ($slider_text2_titles) { $i = 6;
                foreach ($slider_text2_titles as $key => $value) {
                    $info=array();
                    $info['content_heading'] = $value;
                    $info['content_data'] = $slider_texts[$key];
                    $this->db->where('content_id', $i);
                    $this->db->where('content_type', 'slider_text2');
                    $this->db->update('content', $info);
                    $i++;
                }
            }
            $slider_text3_titles = $this->input->post('slider_text3_title');
            $slider_texts = $this->input->post('slider_text3');
            if ($slider_text3_titles) { $i = 7;
                foreach ($slider_text3_titles as $key => $value) {
                    $info=array();
                    $info['content_heading'] = $value;
                    $info['content_data'] = $slider_texts[$key];
                    $this->db->where('content_id', $i);
                    $this->db->where('content_type', 'slider_text3');
                    $this->db->update('content', $info);
                    $i++;
                }
            }
        ...
        $this->session->set_flashdata('message', $message);
        redirect(base_url('index.php/admin/system_control/view_settings'));
    }
